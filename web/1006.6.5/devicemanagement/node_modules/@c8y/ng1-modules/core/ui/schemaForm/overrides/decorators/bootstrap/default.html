...
>
  ...
  <input
    ...
    name="{{ form.key.slice(-1)[0] + $id }}"
    aria-describedby="{{ form.key.slice(-1)[0] + 'Status' + $id }}"
  />
  <div
    ng-if="form.fieldAddonLeft || form.fieldAddonRight"
  ...
  ></span>
  <span
    ng-if="hasError() || hasSuccess()"
    id="{{ form.key.slice(-1)[0] + 'Status' + $id }}"
    class="sr-only"
  >
    <span ng-if="hasSuccess()"> ({{ 'valid`value`' | translate }}) </span>
    <span ng-if="hasError()"> ({{ 'invalid`value`' | translate }}) </span>
  </span>
