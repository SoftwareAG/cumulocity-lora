...
export class CustomCodec {
  ...
  constructor(mo: IManagedObject) {
    ...
    try {
      this.vm = new NodeVM({
        eval: false,
        require: {
          external: ["lora-codec-interface"],
          import: ["lora-codec-interface"],
        },
      });
    } catch (e) {
  ...
  public set decodeString(newDecodeString: string) {
    ...
    try {
      this.decodeScript = new VMScript(
        "const { C8YData } = require('lora-codec-interface');module.exports = function(device, fport, time, payload) { " +
          newDecodeString +
          " }"
      ).compile();
    } catch (e) {
  ...
  public set encodeString(newEncodeString: string) {
    console.log("Compiling script...");
    try {
      this.encodeScript = new VMScript(
        "const { DownlinkData } = require('lora-codec-interface');module.exports = function(device, operation) { " +
          newEncodeString +
          " }"
      ).compile();
    } catch (e) {
